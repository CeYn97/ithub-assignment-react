...
interface Form {
  children: React.ReactElement<{ name: keyof FormData }> | React.ReactElement<{ name: keyof FormData }>[]
  title: string
  schema: ZodObject
}
...
export default function Form({ children, title, schema }: Form) {
  ...
  const fieldNames = Children.map(children, (child) => child.props.name).filter(Boolean)
